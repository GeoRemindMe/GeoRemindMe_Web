...
class _Session_RPC(_Session_Dict, db.Model):
    '''
        Para conexion con el movil, etc.
        No almacena datos de sesion, solo el usuario y su idioma
    '''
    expires = db.DateTimeProperty()
    created = db.DateTimeProperty(auto_now_add=True)
    _user = db.ReferenceProperty(User, collection_name='session_rpc')
    _language_code = db.TextProperty()
    ...
    def restore_session(cls, session_id):
        '''
            ...
            Comprueba que la fecha sea valida
                ...
                :returns: :class:`_Session_Data` o None
        '''
        session = _Session_RPC.get_by_key_name(session_id)
        if session is not None:
            if session.is_valid():
                return session
        return None
