...
def new_list(sender, **kwargs):
    if isinstance(sender, ListAlert):
        timeline = UserTimelineSystem(parent=sender.user.key(), msg_id=250, instance=sender)
    elif isinstance(sender, ListUser):
        timeline = UserTimelineSystem(parent=sender.user.key(), msg_id=150, instance=sender)
        ...
        timelinePublic.put()
        timeline = UserTimelineSystem(parent=sender.user, msg_id=350, instance=sender)
        ...
        timelinePublic.put()
        timeline = UserTimelineSystem(parent=sender.user, msg_id=360, instance=sender)
    else:
        return
    timeline.put()
...
def modified_list(sender, **kwargs):
    if isinstance(sender, ListRequested):
        timeline = UserTimelineSystem(parent=kwargs['querier'], msg=351, _vis=sender._get_visibility())
...
def new_following_list(sender, **kwargs):
    timeline = UserTimelineSystem(parent = kwargs['user'], instance = sender, msg_id=353, visible=False)
...
def deleted_following_list(sender, **kwargs):
    timeline = UserTimelineSystem(parent = kwargs['user'], instance = sender, msg_id=354, visible=False)
    put = db.put_async(timeline)
    sender.counters.set_followers(-1)
    put.get_result()
