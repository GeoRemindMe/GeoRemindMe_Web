...
import libs.atom.url
import libs.gdata.oauth as oauth
import libs.gdata.oauth.rsa as oauth_rsa
import libs.gdata.tlslite.utils.keyfactory as keyfactory
import libs.gdata.tlslite.utils.cryptomath as cryptomath
...
def generate_client_login_request_body(email, password, service, source, 
  ...
  """
  return libs.gdata.gauth.generate_client_login_request_body(email, password,
...
def get_client_login_token(http_body):
  ...
  """
  return libs.gdata.gauth.get_client_login_token_string(http_body)
...
def extract_client_login_token(http_body, scopes):
  ...
  Args:
    ...
    scopes: list containing libs.atom.url.Urls or strs. The scopes list contains
...
def get_captcha_challenge(http_body, 
  ...
  """
  return libs.gdata.gauth.get_captcha_challenge(http_body, captcha_base_url)
...
def GenerateOAuthRequestTokenUrl(
  ...
  Returns:
    libs.atom.url.Url OAuth request token URL.
  """
  ...
  return libs.atom.url.parse_url(oauth_request.to_url())
...
def GenerateOAuthAuthorizationUrl(
  ...
  Returns:
    libs.atom.url.Url OAuth authorization URL.
  """
...
def GenerateOAuthAccessTokenUrl(
  ...
  Args:
    authorized_request_token: libs.gdata.auth.OAuthToken OAuth authorized request
        token.
    oauth_input_params: OAuthInputParams OAuth input parameters.    
  ...
  Returns:
    libs.atom.url.Url OAuth access token URL.
  """
...
def generate_auth_sub_url(next, scopes, secure=False, session=True,
  ...
  Args:
    next: libs.atom.url.URL or string The URL user will be sent to after
    ...
    session: boolean (optional) Determines whether or not the issued token
             can be upgraded to a session token.
    request_url: libs.atom.url.Url or str The beginning of the request URL. This
...
def extract_auth_sub_token_from_url(url, 
  ...
  Args:
    url: libs.atom.url.Url or str representing the current URL. The token value
  ...
  if isinstance(url, (str, unicode)):
    url = libs.atom.url.parse_url(url)
  ...
  def __init__(self, signature_method, consumer_key, consumer_secret=None,
    ...
    Args:
      signature_method: class which provides implementation for strategy class
          ...
          libs.gdata.auth.OAuthSignatureMethod.HMAC_SHA1. Instead of passing in
...
class ClientLoginToken(libs.atom.http_interface.GenericToken):
  ...
  Args:
    ...
    scopes: list of str or libs.atom.url.Url specifying the beginnings of URLs
  ...
  def valid_for_scope(self, url):
    ...
    for scope in self.scopes:
      if scope == libs.atom.token_store.SCOPE_ALL:
        return True
      if isinstance(scope, (str, unicode)):
        scope = libs.atom.url.parse_url(scope)
      if scope == url:
        return True
...
class OAuthToken(libs.atom.http_interface.GenericToken):
  ...
  def GetAuthHeader(self, http_method, http_url, realm=''):
    ...
    Args:
      ...
      http_url: string or libs.atom.url.Url HTTP URL to which request is made.
    ...
    if isinstance(http_url, types.StringTypes):
      http_url = libs.atom.url.parse_url(http_url)
    header = None
    token = None
