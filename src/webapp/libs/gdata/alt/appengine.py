...
class AppEngineHttpClient(libs.atom.http_interface.GenericHttpClient):
...
class AppEngineTokenStore(libs.atom.token_store.TokenStore):
  ...
  def find_token(self, url):
    ...
    Returns:
      ...
      token in the datastore, a libs.atom.http_interface.GenericToken is returned.
    """
    if url is None:
      return None
    ...
    for scope, token in tokens.iteritems():
      if token.valid_for_scope(url):
        return token
    return libs.atom.http_interface.GenericToken()
