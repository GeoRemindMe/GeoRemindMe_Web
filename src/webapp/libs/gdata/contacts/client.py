...
import libs.gdata.client
import libs.gdata.contacts.data
import libs.atom.data
...
class ContactsClient(libs.gdata.client.GDClient):
  api_version = '3'
  auth_service = 'cp'
  server = "www.google.com"
  contact_list = "default"
  auth_scopes = libs.gdata.gauth.AUTH_SCOPES['cp']
  ssl = True
  ...
  def __init__(self, domain=None, auth_token=None, **kwargs):
    ...
    Args:
      ...
      kwargs: The other parameters to pass to the libs.gdata.client.GDClient
          constructor.
    """
    super(ContactsClient, self).__init__(auth_token=auth_token, **kwargs)
    #libs.gdata.client.GDClient.__init__(self, auth_token=auth_token, **kwargs)
    self.domain = domain
  ...
  def get_contact(self, uri, desired_class=libs.gdata.contacts.data.ContactEntry,
  ...
  def create_contact(self, new_contact, insert_uri=None,  auth_token=None,  **kwargs):
    ...
    Args:
      new_contact: libs.atom.Entry or subclass A new contact which is to be added to
                Google Contacts.
  ...
  def add_contact(self, new_contact, insert_uri=None, auth_token=None,  
    ...
    contact = libs.gdata.contacts.data.ContactEntry()
    
    if billing_information is not None:
      if not isinstance(billing_information, libs.gdata.contacts.data.BillingInformation):
        billing_information = libs.gdata.contacts.data.BillingInformation(text=billing_information) 
      
      contact.billing_information = billing_information

    if birthday is not None:
      if not isinstance(birthday, libs.gdata.contacts.data.Birthday):
        birthday = libs.gdata.contacts.data.Birthday(when=birthday)
      
      contact.birthday = birthday 
    
    if calendar_link is not None:
      if type(calendar_link) is not type(list()):
        calendar_link = [calendar_link]
      
      for link in calendar_link:
        if not isinstance(link, libs.gdata.contacts.data.CalendarLink):
          if type(link) is not type(dict()):
            raise TypeError, "calendar_link Requires dictionary not %s" % type(link)
        
          link = libs.gdata.contacts.data.CalendarLink(
  ...
  def get_contacts(self,  desired_class=libs.gdata.contacts.data.ContactsFeed,
                   auth_token=None, **kwargs):
  ...
  def get_group(self, uri=None, desired_class=libs.gdata.contacts.data.GroupEntry,
                auth_token=None, **kwargs):
  ...
  def get_groups(self, uri=None, desired_class=libs.gdata.contacts.data.GroupsFeed,
                 auth_token=None, **kwargs):
  ...
  def change_photo(self, media, contact_entry_or_url, content_type=None, 
    ...
    Args:
      media: filename, file-like-object, or a libs.gdata.MediaSource object to send.
  ...
  def get_photo(self, contact_entry_or_url):
    ...
    Args:
      contact_entry_or_url: a libs.gdata.contacts.ContactEntry objecr or a string
  ...
  def get_profiles_feed(self, uri=None):
    ...
    return self.Get(uri,
                    desired_class=libs.gdata.contacts.data.ProfilesFeed)
  ...
  def get_profile(self, uri):
    ...
    """
    return self.Get(uri,
                    desired_class=libs.gdata.contacts.data.ProfileEntry)
  ...
  def update_profile(self, updated_profile, auth_token=None, force=False, **kwargs):
    ...
    Args:
      updated_profile: libs.atom.Entry or subclass containing
  ...
  def execute_batch(self, batch_feed, url=DEFAULT_BATCH_URL, desired_class=None):
    ...
    Args:
      batch_feed: libs.gdata.contacts.ContactFeed A feed containing batch
  ...
  def execute_batch_profiles(self, batch_feed, url,
    ...
    Args:
      batch_feed: libs.gdata.profiles.ProfilesFeed A feed containing batch
          request entries. Each entry contains the operation to be performed
          on the data contained in the entry. For example an entry with an
          operation type of insert will be used as if the individual entry
          had been inserted.
      ...
      converter: Function (optional) The function used to convert the server's
          ...
          libs.gdata.profiles.ProfilesFeedFromString.
...
class ContactsQuery(libs.gdata.client.Query):
  """ 
  ...
  def __init__(self, feed=None, group=None, orderby=None, showdeleted=None,
    ...
    """
    libs.gdata.client.Query.__init__(self, **kwargs)
    self.group = group
  ...
  def modify_request(self, http_request):
    if self.group:
      libs.gdata.client._add_query_param('group', self.group, http_request)
    if self.orderby:
      libs.gdata.client._add_query_param('orderby', self.orderby, http_request)
    if self.sortorder:
      libs.gdata.client._add_query_param('sortorder', self.sortorder, http_request)
    if self.showdeleted:
      libs.gdata.client._add_query_param('showdeleted', self.showdeleted, http_request)
    libs.gdata.client.Query.modify_request(self, http_request)

  ModifyRequest = modify_request
    

class ProfilesQuery(libs.gdata.client.Query):
