...
import libs.atom.service
import libs.gdata
import libs.atom
import libs.atom.http_interface
import libs.atom.token_store
import libs.gdata.auth
import libs.gdata.gauth
...
CLIENT_LOGIN_SCOPES = libs.gdata.gauth.AUTH_SCOPES
...
http_request_handler = libs.atom.service
...
class GDataService(libs.atom.service.AtomService):
  ...
  def __init__(self, email=None, password=None, account_type='HOSTED_OR_GOOGLE',
    ...
    Args:
      ...
      http_client: An object responsible for making HTTP requests using a
          ...
          libs.atomhttp.ProxiedHttpClient will be used.
      token_store: Keeps a collection of authorization tokens which can be
          ...
          find_token based on a URL (libs.atomurl.Url or a string), add_token,
          and remove_token.
    """
    libs.atom.service.AtomService.__init__(self, http_client=http_client, 
    ...
    if http_request_handler.__name__ == 'libs.gdata.urlfetch':
      import libs.gdata.alt.appengine
      self.http_client = libs.gdata.alt.appengine.AppEngineHttpClient()
  ...
  def SetOAuthInputParameters(self, signature_method, consumer_key,
    ...
    Args:
      signature_method: class which provides implementation for strategy class
          ...
          constants defined by libs.gdata.auth.OAuthSignatureMethod. Currently
          they are libs.gdata.auth.OAuthSignatureMethod.RSA_SHA1 and
          libs.gdata.auth.OAuthSignatureMethod.HMAC_SHA1
  ...
  def FetchOAuthRequestToken(self, scopes=None, extra_parameters=None,
    ...
    Returns:
      The fetched request token as a libs.gdata.auth.OAuthToken object.
    ...
    if oauth_callback:
      if extra_parameters is not None:
        ...
        extra_parameters = {'oauth_callback': oauth_callback}
    request_token_url = libs.gdata.auth.GenerateOAuthRequestTokenUrl(
    ...
    if response.status == 200:
      token = libs.gdata.auth.OAuthToken()
  ...
  def SetOAuthToken(self, oauth_token):
    ...
    Args:
      request_token: libs.gdata.auth.OAuthToken OAuth request token.
    """
  ...
  def GenerateOAuthAuthorizationURL(
    ...
    Args:
      request_token: libs.gdata.auth.OAuthToken (optional) OAuth request token.
          If not specified, then the current token will be used if it is of
          type <libs.gdata.auth.OAuthToken>, else it is found by looking in the
    ...
    """
    if request_token and not isinstance(request_token, libs.gdata.auth.OAuthToken):
      raise NonOAuthToken
    if not request_token:
      if isinstance(self.current_token, libs.gdata.auth.OAuthToken):
        request_token = self.current_token
      else:
        current_scopes = lookup_scopes(self.service)
        if current_scopes:
          token = self.token_store.find_token(current_scopes[0])
          if isinstance(token, libs.gdata.auth.OAuthToken):
            request_token = token
    if not request_token:
      raise NonOAuthToken
    return str(libs.gdata.auth.GenerateOAuthAuthorizationUrl(
        request_token,
  ...
  def UpgradeToOAuthAccessToken(self, authorized_request_token=None,
    ...
    Args:
      authorized_request_token: libs.gdata.auth.OAuthToken (optional) OAuth request
          token. If not specified, then the current token will be used if it is
          of type <libs.gdata.auth.OAuthToken>, else it is found by looking in the
          token_store by looking for a token for the current scope.
    ...
    if (authorized_request_token and
        not isinstance(authorized_request_token, libs.gdata.auth.OAuthToken)):
      raise NonOAuthToken
    ...
    if not authorized_request_token:
      raise NonOAuthToken
    access_token_url = libs.gdata.auth.GenerateOAuthAccessTokenUrl(
    ...
    if response.status == 200:
      token = libs.gdata.auth.OAuthTokenFromHttpBody(response.read())
  ...
  def RevokeOAuthToken(self, request_url='%s/accounts/AuthSubRevokeToken' % \
    ...
    if not isinstance(token, libs.gdata.auth.OAuthToken):
      raise NonOAuthToken
  ...
  def GetAuthSubToken(self):
    ...
    """
    if isinstance(self.current_token, libs.gdata.auth.AuthSubToken):
      return self.current_token.get_token_string()
    current_scopes = lookup_scopes(self.service)
    if current_scopes:
      token = self.token_store.find_token(current_scopes[0])
      if isinstance(token, libs.gdata.auth.AuthSubToken):
        return token.get_token_string()
    else:
      token = self.token_store.find_token(libs.atom.token_store.SCOPE_ALL)
      if isinstance(token, libs.gdata.auth.ClientLoginToken):
        return token.get_token_string()
      return None
  ...
  def SetAuthSubToken(self, token, scopes=None, rsa_key=None):
    ...
    Args:
     token: libs.gdata.auth.AuthSubToken or libs.gdata.auth.SecureAuthSubToken or string
    ...
    """
    if not isinstance(token, libs.gdata.auth.AuthSubToken):
      token_string = token
      if rsa_key:
        token = libs.gdata.auth.SecureAuthSubToken(rsa_key)
      else:
        token = libs.gdata.auth.AuthSubToken()
    ...
    if not token.scopes:
      if scopes is None:
        scopes = lookup_scopes(self.service)
        if scopes is None:
          scopes = [libs.atom.token_store.SCOPE_ALL]
      token.scopes = scopes
    if self.auto_set_current_token:
  ...
  def GetClientLoginToken(self):
    ...
    """
    if isinstance(self.current_token, libs.gdata.auth.ClientLoginToken):
      return self.current_token.get_token_string()
    current_scopes = lookup_scopes(self.service)
    if current_scopes:
      token = self.token_store.find_token(current_scopes[0])
  ...
  def SetClientLoginToken(self, token, scopes=None):
    ...
    """
    if not isinstance(token, libs.gdata.auth.ClientLoginToken):
      token_string = token
      token = libs.gdata.auth.ClientLoginToken()
      token.set_token_string(token_string)
  ...
  def __SetSource(self, new_source):
    ...
    self.additional_headers['User-Agent'] = libs.atom.http_interface.USER_AGENT % (
        self.__source,)
  ...
  def ProgrammaticLogin(self, captcha_token=None, captcha_response=None):
    ...
    """
    request_body = libs.gdata.auth.generate_client_login_request_body(self.email,
    ...
    if auth_response.status == 200:
      ...
      self.SetClientLoginToken(
          libs.gdata.auth.get_client_login_token(response_body))
      self.__captcha_token = None
      self.__captcha_url = None
      ...
      captcha_parameters = libs.gdata.auth.get_captcha_challenge(response_body,
  ...
  def GenerateAuthSubURL(self, next, scope, secure=False, session=True, 
    ...
    if not isinstance(scope, (list, tuple)):
      scope = (scope,)
    return libs.gdata.auth.generate_auth_sub_url(next, scope, secure=secure, 
        session=session, 
  ...
  def UpgradeToSessionToken(self, token=None):
    ...
    Args:
      token: A libs.gdata.auth.AuthSubToken or libs.gdata.auth.SecureAuthSubToken
  ...
  def upgrade_to_session_token(self, token):
    ...
    Returns:
      The upgraded token as a libs.gdata.auth.AuthSubToken object.
    ...
    if response.status == 200:
      token.set_token_string(
          libs.gdata.auth.token_from_http_body(response_body))
      return token
    else:
      raise TokenUpgradeFailed({'status': response.status,
                                'reason': 'Non 200 response on upgrade',
  ...
  def Get(self, uri, extra_headers=None, redirects_remaining=4, 
    ...
    if server_response.status == 200:
      ...
      feed = libs.gdata.GDataFeedFromString(result_body)
      if not feed:
        # If conversion to a GDataFeed failed, try to convert the server's
        # response to a GDataEntry.
        entry = libs.gdata.GDataEntryFromString(result_body)
        if not entry:
  ...
  def GetMedia(self, uri, extra_headers=None):
    ...
    response_handle = self.request('GET', uri,
        headers=extra_headers)
    return libs.gdata.MediaSource(response_handle, response_handle.getheader(
            'Content-Type'),
  ...
  def GetEntry(self, uri, extra_headers=None):
    ...
    See also documentation for libs.gdata.service.Get
    ...
    result = GDataService.Get(self, uri, extra_headers, 
        converter=libs.atom.EntryFromString)
    if isinstance(result, libs.atom.Entry):
      return result
    else:
      raise UnexpectedReturnType, 'Server did not send an entry' 

  def GetFeed(self, uri, extra_headers=None, 
              converter=libs.gdata.GDataFeedFromString):
    ...
    if isinstance(result, libs.atom.Feed):
      return result
    else:
  ...
  def GetNext(self, feed):
    ...
    Args:
      feed: libs.atomFeed or a subclass. The feed should contain a next link and
    ...
    def ConvertToFeedClass(xml_string):
      return libs.atom.CreateClassFromXMLString(feed.__class__, xml_string)
  ...
  def Post(self, data, uri, extra_headers=None, url_params=None,
    ...
    Args:
      data: string, ElementTree._Element, libs.atomEntry, or libs.gdata.GDataEntry The
  ...
  def PostOrPut(self, verb, data, uri, extra_headers=None, url_params=None, 
    ...
    elif media_source or isinstance(data, libs.gdata.MediaSource):
      if isinstance(data, libs.gdata.MediaSource):
        media_source = data
...
def ExtractToken(url, scopes_included_in_next=True):
  ...
  token = libs.gdata.auth.AuthSubTokenFromUrl(parsed[4])
  scopes = ''
...
def GenerateAuthSubRequestUrl(next, scopes, hd='default', secure=False,
  ...
  if include_scopes_in_next:
    if next.find('?') > -1:
      ...
      next += '?%s' % urllib.urlencode({SCOPE_URL_PARAM_NAME:scope})
  return libs.gdata.auth.GenerateAuthSubUrl(next=next, scope=scope, secure=secure,
...
class Query(dict):
  ...
  def ToUri(self):
    ...
    return libs.atom.service.BuildUri(q_feed, self)
