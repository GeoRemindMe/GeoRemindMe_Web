...
GACL_NAMESPACE = libs.gdata.GACL_NAMESPACE
GACL_TEMPLATE = libs.gdata.GACL_TEMPLATE



class ValueAttributeContainer(libs.atom.AtomBase):
  """A parent class for all Calendar classes which have a value attribute.

  Children include Color, AccessLevel, Hidden
  """

  _children = libs.atom.AtomBase._children.copy()
  _attributes = libs.atom.AtomBase._attributes.copy()
...
class Where(libs.atom.AtomBase):
  """The Google Calendar Where element"""

  _tag = 'where'
  _namespace = libs.gdata.GDATA_NAMESPACE
  _children = libs.atom.AtomBase._children.copy()
  _attributes = libs.atom.AtomBase._attributes.copy()
...
class CalendarListEntry(libs.gdata.GDataEntry, libs.gdata.LinkFinder):
  """A Google Calendar meta Entry flavor of an Atom Entry """

  _tag = libs.gdata.GDataEntry._tag
  _namespace = libs.gdata.GDataEntry._namespace
  _children = libs.gdata.GDataEntry._children.copy()
  _attributes = libs.gdata.GDataEntry._attributes.copy()
  ...
  _children['{%s}where' % libs.gdata.GDATA_NAMESPACE] = ('where', Where)
  ...
  def __init__(self, author=None, category=None, content=None,
    ...
    libs.gdata.GDataEntry.__init__(self, author=author, category=category,
...
class CalendarListFeed(libs.gdata.GDataFeed, libs.gdata.LinkFinder):
  """A Google Calendar meta feed flavor of an Atom Feed"""

  _tag = libs.gdata.GDataFeed._tag
  _namespace = libs.gdata.GDataFeed._namespace
  _children = libs.gdata.GDataFeed._children.copy()
  _attributes = libs.gdata.GDataFeed._attributes.copy()
  _children['{%s}entry' % libs.atom.ATOM_NAMESPACE] = ('entry', [CalendarListEntry])


class Scope(libs.atom.AtomBase):
...
class CalendarAclEntry(libs.gdata.GDataEntry, libs.gdata.LinkFinder):
...
class CalendarAclFeed(libs.gdata.GDataFeed, libs.gdata.LinkFinder):
  ...
  _children = libs.gdata.GDataFeed._children.copy()
  _attributes = libs.gdata.GDataFeed._attributes.copy()
  _children['{%s}entry' % libs.atom.ATOM_NAMESPACE] = ('entry', [CalendarAclEntry])


class CalendarEventCommentEntry(libs.gdata.GDataEntry, libs.gdata.LinkFinder):
...
class CalendarEventCommentFeed(libs.gdata.GDataFeed, libs.gdata.LinkFinder):
  ...
  _children = libs.gdata.GDataFeed._children.copy()
  _attributes = libs.gdata.GDataFeed._attributes.copy()
  _children['{%s}entry' % libs.atom.ATOM_NAMESPACE] = ('entry',
      [CalendarEventCommentEntry])


class ExtendedProperty(libs.gdata.ExtendedProperty):
  """A transparent subclass of libs.gdata.ExtendedProperty added to this module
  for backwards compatibility."""


class Reminder(libs.atom.AtomBase):
...
class When(libs.atom.AtomBase):
  ...
  _children = libs.atom.AtomBase._children.copy()
  _attributes = libs.atom.AtomBase._attributes.copy()
  _children['{%s}reminder' % libs.gdata.GDATA_NAMESPACE] = ('reminder', [Reminder])
...
class Recurrence(libs.atom.AtomBase):
...
class UriEnumElement(libs.atom.AtomBase):
  ...
  def _ConvertElementAttributeToMember(self, attribute, value):
    ...
    if self.__class__._attributes.has_key(attribute):
      ...
      libs.atom.ExtensionContainer._ConvertElementAttributeToMember(self,
                                                               attribute,
                                                               value)
  ...
  def _AddMembersToElementTree(self, tree):
    ...
    # extension elements.
    libs.atom.ExtensionContainer._AddMembersToElementTree(self, tree)
...
class Comments(libs.atom.AtomBase):
  ...
  _children = libs.atom.AtomBase._children.copy()
  _attributes = libs.atom.AtomBase._attributes.copy()
  _children['{%s}feedLink' % libs.gdata.GDATA_NAMESPACE] = ('feed_link',
                                                       libs.gdata.FeedLink)
  _attributes['rel'] = 'rel'
...
class Who(UriEnumElement):
  ...
  _children = UriEnumElement._children.copy()
  _attributes = UriEnumElement._attributes.copy()
  _children['{%s}attendeeStatus' % libs.gdata.GDATA_NAMESPACE] = (
      'attendee_status', AttendeeStatus)
  _children['{%s}attendeeType' % libs.gdata.GDATA_NAMESPACE] = ('attendee_type',
                                                           AttendeeType)
...
class OriginalEvent(libs.atom.AtomBase):
  ...
  _children['{%s}when' % libs.gdata.GDATA_NAMESPACE] = ('when', When)
  _attributes['id'] = 'id'
...
class CalendarEventEntryLink(libs.gdata.EntryLink):
  ...
  _tag = 'entryLink'
  _namespace = libs.gdata.GDATA_NAMESPACE
  _children = libs.gdata.EntryLink._children.copy()
  _attributes = libs.gdata.EntryLink._attributes.copy()
...
class RecurrenceException(libs.atom.AtomBase):
  ...
  _children = libs.atom.AtomBase._children.copy()
  _attributes = libs.atom.AtomBase._attributes.copy()
  _children['{%s}entryLink' % libs.gdata.GDATA_NAMESPACE] = ('entry_link',
      CalendarEventEntryLink)
  _children['{%s}originalEvent' % libs.gdata.GDATA_NAMESPACE] = ('original_event',
                                                            OriginalEvent)
...
class SendEventNotifications(libs.atom.AtomBase):
...
class QuickAdd(libs.atom.AtomBase):
  ...
  def _TransferToElementTree(self, element_tree):
    ...
    libs.atom.AtomBase._TransferToElementTree(self, element_tree)
    return element_tree
  ...
  def _TakeAttributeFromElementTree(self, attribute, element_tree):
    if attribute == 'value':
      ...
      libs.atom.AtomBase._TakeAttributeFromElementTree(self, attribute,
          element_tree)


class SyncEvent(libs.atom.AtomBase):
  _tag = 'syncEvent'
  _namespace = GCAL_NAMESPACE
  _children = libs.atom.AtomBase._children.copy()
  _attributes = libs.atom.AtomBase._attributes.copy()
...
class Sequence(libs.atom.AtomBase):
  _tag = 'sequence'
  _namespace = GCAL_NAMESPACE
  _children = libs.atom.AtomBase._children.copy()
  _attributes = libs.atom.AtomBase._attributes.copy()
...
class WebContentGadgetPref(libs.atom.AtomBase):
...
class WebContent(libs.atom.AtomBase):
...
class WebContentLink(libs.atom.Link):

  _tag = 'link'
  _namespace = libs.atom.ATOM_NAMESPACE
  _children = libs.atom.Link._children.copy()
  _attributes = libs.atom.Link._attributes.copy()
  _children['{%s}webContent' % GCAL_NAMESPACE] = ('web_content', WebContent)

  def __init__(self, title=None, href=None, link_type=None,
        web_content=None):
    libs.atom.Link.__init__(self, rel=WEB_CONTENT_LINK_REL, title=title, href=href,
        link_type=link_type)
    self.web_content = web_content


class GuestsCanInviteOthers(libs.atom.AtomBase):
  ...
  def __init__(self, value='true', *args, **kwargs):
    libs.atom.AtomBase.__init__(self, *args, **kwargs)
    self.value = value


class GuestsCanSeeGuests(libs.atom.AtomBase):
...
class GuestsCanModify(libs.atom.AtomBase):
...
class CalendarEventEntry(libs.gdata.BatchEntry):
  """A Google Calendar flavor of an Atom Entry """

  _tag = libs.gdata.BatchEntry._tag
  _namespace = libs.gdata.BatchEntry._namespace
  _children = libs.gdata.BatchEntry._children.copy()
  _attributes = libs.gdata.BatchEntry._attributes.copy()
  ...
  _children['{%s}where' % libs.gdata.GDATA_NAMESPACE] = ('where', [Where])
  _children['{%s}when' % libs.gdata.GDATA_NAMESPACE] = ('when', [When])
  _children['{%s}who' % libs.gdata.GDATA_NAMESPACE] = ('who', [Who])
  _children['{%s}extendedProperty' % libs.gdata.GDATA_NAMESPACE] = (
      'extended_property', [ExtendedProperty])
  _children['{%s}visibility' % libs.gdata.GDATA_NAMESPACE] = ('visibility',
                                                         Visibility)
  _children['{%s}transparency' % libs.gdata.GDATA_NAMESPACE] = ('transparency',
                                                           Transparency)
  _children['{%s}eventStatus' % libs.gdata.GDATA_NAMESPACE] = ('event_status',
                                                          EventStatus)
  _children['{%s}recurrence' % libs.gdata.GDATA_NAMESPACE] = ('recurrence',
                                                         Recurrence)
  _children['{%s}recurrenceException' % libs.gdata.GDATA_NAMESPACE] = (
  ...
  _children['{%s}comments' % libs.gdata.GDATA_NAMESPACE] = ('comments', Comments)
  _children['{%s}originalEvent' % libs.gdata.GDATA_NAMESPACE] = ('original_event',
                                                            OriginalEvent)
  ...
  def __init__(self, author=None, category=None, content=None,
    ...
    libs.gdata.BatchEntry.__init__(self, author=author, category=category,
                        content=content,
  ...
  def _ConvertElementTreeToMember(self, child_tree):
    ...
    if (child_tree.tag == '{%s}link' % libs.atom.ATOM_NAMESPACE and
        child_tree.attrib['rel'] == WEB_CONTENT_LINK_REL):
      if self.link is None:
        self.link = []
      self.link.append(libs.atom._CreateClassFromElementTree(WebContentLink,
                                                        child_tree))
      return
    ...
    if self.__class__._children.has_key(child_tree.tag):
      ...
      if isinstance(member_class, list):
        ...
        getattr(self, member_name).append(libs.atom._CreateClassFromElementTree(
            member_class[0], child_tree))
      else:
        setattr(self, member_name,
                libs.atom._CreateClassFromElementTree(member_class, child_tree))
    else:
      libs.atom.ExtensionContainer._ConvertElementTreeToMember(self, child_tree)
  ...
  def GetWebContentLink(self):
    ...
    Returns:
      A libs.gdata.calendar.WebContentLink or none if none of the links had rel
...
def CalendarEventEntryFromString(xml_string):
  return libs.atom.CreateClassFromXMLString(CalendarEventEntry, xml_string)


def CalendarEventCommentEntryFromString(xml_string):
  return libs.atom.CreateClassFromXMLString(CalendarEventCommentEntry, xml_string)


CalendarEventEntryLink._children = {'{%s}entry' % libs.atom.ATOM_NAMESPACE:
    ('entry', CalendarEventEntry)}


def CalendarEventEntryLinkFromString(xml_string):
  return libs.atom.CreateClassFromXMLString(CalendarEventEntryLink, xml_string)


class CalendarEventFeed(libs.gdata.BatchFeed, libs.gdata.LinkFinder):
  """A Google Calendar event feed flavor of an Atom Feed"""

  _tag = libs.gdata.BatchFeed._tag
  _namespace = libs.gdata.BatchFeed._namespace
  _children = libs.gdata.BatchFeed._children.copy()
  _attributes = libs.gdata.BatchFeed._attributes.copy()
  _children['{%s}entry' % libs.atom.ATOM_NAMESPACE] = ('entry',
                                                  [CalendarEventEntry])
  _children['{%s}timezone' % GCAL_NAMESPACE] = ('timezone', Timezone)
  ...
  def __init__(self, author=None, category=None, contributor=None,
      ...
      extension_elements=None, extension_attributes=None, text=None):
     libs.gdata.BatchFeed.__init__(self, author=author, category=category,
...
def CalendarListEntryFromString(xml_string):
  return libs.atom.CreateClassFromXMLString(CalendarListEntry, xml_string)


def CalendarAclEntryFromString(xml_string):
  return libs.atom.CreateClassFromXMLString(CalendarAclEntry, xml_string)


def CalendarListFeedFromString(xml_string):
  return libs.atom.CreateClassFromXMLString(CalendarListFeed, xml_string)


def CalendarAclFeedFromString(xml_string):
  return libs.atom.CreateClassFromXMLString(CalendarAclFeed, xml_string)


def CalendarEventFeedFromString(xml_string):
  return libs.atom.CreateClassFromXMLString(CalendarEventFeed, xml_string)


def CalendarEventCommentFeedFromString(xml_string):
  return libs.atom.CreateClassFromXMLString(CalendarEventCommentFeed, xml_string)
