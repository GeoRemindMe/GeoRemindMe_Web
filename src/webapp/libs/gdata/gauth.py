...
def _to_uri(str_or_uri):
  if isinstance(str_or_uri, (str, unicode)):
    return libs.atom.http_core.Uri.parse_uri(str_or_uri)
  return str_or_uri


def generate_auth_sub_url(next, scopes, secure=False, session=True,
    request_url=libs.atom.http_core.parse_uri(
  ...
  Args:
    next: libs.atom.http_core.Uri or string The URL user will be sent to after
          authorizing this web application to access their data.
    scopes: list containint strings or libs.atom.http_core.Uri objects. The URLs
            of the services to be accessed. Could also be a single string
            or single libs.atom.http_core.Uri for requesting just one scope.
    secure: boolean (optional) Determines whether or not the issued token
            is a secure token.
    session: boolean (optional) Determines whether or not the issued token
             can be upgraded to a session token.
    request_url: libs.atom.http_core.Uri or str The beginning of the request URL.
                 This is normally
  ...
  Returns:
    An libs.atom.http_core.Uri which the user's browser should be directed to in
    order to authorize this application to access their information.
  """
  if isinstance(next, (str, unicode)):
    next = libs.atom.http_core.Uri.parse_uri(next)
  # If the user passed in a string instead of a list for scopes, convert to
  # a single item tuple.
  if isinstance(scopes, (str, unicode, libs.atom.http_core.Uri)):
    scopes = (scopes,)
  ...
  if isinstance(request_url, (str, unicode)):
    request_url = libs.atom.http_core.Uri.parse_uri(request_url)
...
def auth_sub_string_from_url(url, scopes_param_prefix='auth_sub_scopes'):
  ...
  Returns:
    ...
    (as libs.atom.http_core.Uri objects) which are URL prefixes under which this
  ...
  if isinstance(url, (str, unicode)):
    url = libs.atom.http_core.Uri.parse_uri(url)
...
class AuthSubToken(object):
  ...
  def from_url(str_or_uri):
    ...
    Args:
      str_or_uri: The current page's URL (as a str or libs.atom.http_core.Uri)
...
class SecureAuthSubToken(AuthSubToken):
  ...
  def modify_request(self, http_request):
    ...
    Args:
      http_request: The libs.atom.http_core.HttpRequest which contains all of the
...
def generate_request_for_request_token(
  ...
  Returns:
    An libs.atom.http_core.HttpRequest object with the URL, Authorization header
    and body filled in.
  """
  request = libs.atom.http_core.HttpRequest(auth_server_url, 'POST')
...
def generate_request_for_access_token(
  ...
  """
  http_request = libs.atom.http_core.HttpRequest(auth_server_url, 'POST')
...
def generate_oauth_authorization_url(
  ...
  Returns:
    An libs.atom.http_core.Uri pointing to the token authorization page where the
    user may allow or deny this app to access their Google data.
  """
  uri = libs.atom.http_core.Uri.parse_uri(auth_server)
