...
class AtomService(object):
  ...
  #@libs.atom.v1_deprecated('Please use libs.atom.client.AtomPubClient instead.')
  def __init__(self, server=None, additional_headers=None, 
    ...
    Args:
      ...
      http_client: An object responsible for making HTTP requests using a
                   ...
                   libs.atom.http.ProxiedHttpClient will be used.
      token_store: Keeps a collection of authorization tokens which can be
                   ...
                   find_token based on a URL (libs.atom.url.Url or a string), add_token,
                   and remove_token.
    """
    self.http_client = http_client or libs.atom.http.ProxiedHttpClient()
    self.token_store = token_store or libs.atom.token_store.TokenStore()
    self.server = server
  ...
  __init__ = libs.atom.v1_deprecated(
      'Please use libs.atom.client.AtomPubClient instead.')(
          __init__)
  ...
  #@libs.atom.v1_deprecated('Please use libs.atom.client.AtomPubClient for requests.')
  def request(self, operation, url, data=None, headers=None, 
      url_params=None):
    if isinstance(url, (str, unicode)):
      if url.startswith('http:') and self.ssl:
        # Force all requests to be https if self.ssl is True.
        url = libs.atom.url.parse_url('https:' + url[5:])
      elif not url.startswith('http') and self.ssl: 
        url = libs.atom.url.parse_url('https://%s%s' % (self.server, url))
      elif not url.startswith('http'):
        url = libs.atom.url.parse_url('http://%s%s' % (self.server, url))
      else:
  ...
  request = libs.atom.v1_deprecated(
      'Please use libs.atom.client.AtomPubClient for requests.')(
          request)
...
class BasicAuthToken(libs.atom.http_interface.GenericToken):
...
def UseBasicAuth(service, username, password, for_proxy=False):
  ...
  Args:
    service: libs.atom.AtomService or a subclass which has an 
...
def ProcessUrl(service, url, for_proxy=False):
  ...
  This method is deprecated, use libs.atom.url.parse_url instead.
  """
  if not isinstance(url, libs.atom.url.Url):
    url = libs.atom.url.parse_url(url)
...
def DictionaryToParamList(url_parameters, escape_params=True):
  ...
  This function is deprcated, use libs.atom.url.Url instead.
...
def HttpRequest(service, operation, data, uri, extra_headers=None, 
  ...
  This method is deprecated, use libs.atom.http.HttpClient.request instead.

  Usage example, perform and HTTP GET on http://www.google.com/:
    import libs.atom.service
    client = libs.atom.service.AtomService()
  ...
  Args:
    service: libs.atom.AtomService object which contains some of the parameters 
...
def __SendDataPart(data, connection):
  """This method is deprecated, use libs.atom.http._send_data_part"""
