...
class ProxyError(libs.atom.http_interface.Error):
  pass
...
class HttpClient(libs.atom.http_interface.GenericHttpClient):
  ...
  def request(self, operation, url, data=None, headers=None):
    ...
    Usage example, perform and HTTP GET on http://www.google.com/:
      ...
      client = libs.atom.http.HttpClient()
    ...
    if data and 'Content-Length' not in all_headers:
      if isinstance(data, types.StringTypes):
        ...
        raise libs.atom.http_interface.ContentLengthRequired('Unable to calculate '
    ...
    if self.v2_http_client is not None:
      http_request = libs.atom.http_core.HttpRequest(method=operation)
      libs.atom.http_core.Uri.parse_uri(str(url)).modify_request(http_request)
    ...
    if not isinstance(url, libs.atom.url.Url):
      if isinstance(url, types.StringTypes):
        ...
        raise libs.atom.http_interface.UnparsableUrlObject('Unable to parse url '
            'parameter because it was not a string or libs.atom.url.Url')
  ...
  def _prepare_connection(self, url, headers):
    ...
    if not proxy_settings:
      ...
      if url.protocol == 'https':
        ...
        proxy_url = libs.atom.url.parse_url(proxy_netloc)
