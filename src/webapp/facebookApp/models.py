...
class Facebook(object):
    ...
    def __init__(self, app_id=OAUTH['facebook']['app_key'],
...
class RealtimeHandler(BaseHandler):
    ...
    def get(self):
        if (self.request.GET.get(u'setup') == u'1' and
            self.user and FACEBOOK_APP['admin_user_ids'].count(self.user.user_id)):
            ...
            self.set_message(type=u'success',
              ...
              self.request.GET.get(u'hub.verify_token') ==
                  FACEBOOK_APP['realtime_verify_token']):
    ...
    def setup_subscription(self):
        path = u'/' + OAUTH['facebook']['app_key'] + u'/subscriptions'
        params = {
            u'access_token': OAUTH['facebook']['app_key'] + u'|' +
                             OAUTH['facebook']['app_secret'],
            u'object': u'user',
            u'fields': _USER_FIELDS,
            u'callback_url': FACEBOOK_APP['external_href'] + u'realtime',
            u'verify_token': FACEBOOK_APP['realtime_verify_token'],
        }
