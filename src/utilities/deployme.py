...
def compress():
    ...
    if os.system("python file_combine_compress.py"):
...
def lang():
    ...
    if os.system("python manage.py compilemessages"):
...
def upload():
    os.chdir( "src/webapp" )
    if os.system("python %s --version=%s update ." % (os.path.join(sys.argv[1],"appcfg.py"),sys.argv[2], ) ):
